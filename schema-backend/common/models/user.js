...
const templateMail = function(title, html) {
  return `<html><head>
        ...
        <a href="${config.url}"><img src="${config.url}/favicon.png" alt="${
    config.name
  }" 
        ...
        <a href="${config.url}">${
    config.name
  }</a>. If you are not sure why you’re receiving this, please contact us.</td>
        </tr></table>
        </body></html>`;
};
...
module.exports = function(User) {
  User.observe("before save", function(context, next) {
    if (context.instance) delete context.instance.unsetAttribute("roles");
    else delete context.data.roles;
    next();
  });
    ...
    User.generateVerificationToken(user, null, function(err, token) {
      ...
      user.save(function(err) {
        ...
        const link =
          ...
          token;
        const html = `<table><tr><td style="padding:15px">Hi <strong>${
          user.name
        }</strong>;<br><br>
                    Thanks so much joining ${
                      config.name
                    }! To finish your register you just need to confirm that we got your email right.
      ...
      if (err) {
        ...
        user.save(function(err) {
          if (err) {
            ...
            const html =
              '<div style="text-align: center"><h3>Good News</h3>' +
              "<p> We have just approved your account. " +
              "You can play " +
              config.name +
              " the game whenever you want.</p></div>";
            transport.sendMail(
              {
                from: from,
                to: user.email,
