  ...
  User.count({}, function(err, count) {
    ...
    User.create(_users, function(err, users) {
        ...
        Role.create(_roles, (err, roles) => {
          ...
          roles.forEach(role => {
            role.principals.create({ principalType: RoleMapping.USER, principalId: users[getRandomInt(users.length-1)].id },
              (err, principal) => {
               if (err) throw err;
        ...
        users[1].projects.create(
          ...
          (err, project) => {
            ...
            Team.create(
              [
                { ownerId: project.ownerId, memberId: users[1].id },
                { ownerId: project.ownerId, memberId: users[2].id },
                { ownerId: project.ownerId, memberId: users[3].id },
                { ownerId: project.ownerId, memberId: users[4].id },
                { ownerId: project.ownerId, memberId: users[5].id }
              ],
              (err, team) => {
                if (err) throw err;
                console.log("Created team:", team);
        ...
        Role.create({ name: "admin" }, (err, role) => {
          ...
          role.principals.create({ principalType: RoleMapping.USER, principalId: users[2].id },
             (err, principal) => {
              if (err) throw err;
              console.log("Created principal:", principal);
            }
